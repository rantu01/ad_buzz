...
export async function updateSpendCap(metaAccountIdRaw, newCapInDollars) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ spend_cap: Math.round(parseFloat(newCapInDollars || 0) * 100) }),
  });
  const data = await res.json();
  if (data.error) throw new Error(`Meta API error: ${data.error.message}`);
  return data;
  ...
  try {
    ...
    for (const acc of allAccounts) {
      ...
      try {
        ...
        const budgetDollars = Number(acc.spendCap || 0);
        if (settings?.autoSpendCapUpdate && budgetDollars > 0) {
          ...
          if (spent >= budgetDollars * 0.95) {
            ...
            try {
              ...
              await updateAdAccount(acc._id.toString(), { spendCap: newCap });
              await db.collection("metaAdAccounts").findOneAndUpdate(
                { metaAccountId: acc.metaAccountId },
                { $set: { spendCap: newCap, updatedAt: new Date() } }
              ).catch(() => {});
            } catch (capErr) {

...
export async function fetchAdAccountsFromBM() {
  ...
  let allAccounts = [];
  let url = `${GRAPH_API_BASE}/${bmId}/owned_ad_accounts?fields=id,name,account_status,currency,balance,spend_cap,amount_spent,disable_reason&limit=100&access_token=${token}`;
    ...
    for (const acc of allAccounts) {
      ...
      try {
        ...
        const updateData = {
          ...
          spendCap: metaAccount?.spendCap || acc.spendCap || 0,
        ...
        const budgetDollars = Number(acc.spendCap || 0) / 100;
        if (settings?.autoSpendCapUpdate && budgetDollars > 0) {
          const spent = insights.spend;
          if (spent >= budgetDollars * 0.95) {
            const newCap = budgetDollars * 1.2;
            try {
              await updateSpendCap(acc.metaAccountId, newCap);
              logEntries.push(`Auto cap updated for ${acc.name}: $${budgetDollars} -> $${newCap}`);
              const newCapCents = Math.round(newCap * 100);
              await updateAdAccount(acc._id.toString(), { spendCap: newCapCents });
              await db.collection("metaAdAccounts").findOneAndUpdate(
                { metaAccountId: acc.metaAccountId },
                { $set: { spendCap: newCapCents, updatedAt: new Date() } }
              ).catch(() => {});
            } catch (capErr) {

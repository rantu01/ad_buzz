...
export async function getOverviewReport() {
  ...
  const [totalUsers, totalDeposits, depositsAgg, totalWithdrawals, withdrawalsAgg, adAccountsAgg] = await Promise.all([
...
export async function getUserActivityReport(startDate, endDate) {
  ...
  const users = await db.collection("users").find({}).project({ uid: 1, email: 1, displayName: 1, phoneNumber: 1, availableBalance: 1, totalEarned: 1, createdAt: 1 }).toArray();

  const uids = users.map((u) => u.uid);
  const [depositCounts, withdrawalCounts] = await Promise.all([
    db.collection("deposits").aggregate([
...
export async function getAdSpendReport(startDate, endDate) {
  ...
  }).project({ name: 1, metaAccountId: 1, accountId: 1, spendCap: 1, spent: 1, currency: 1, status: 1, uid: 1, email: 1, lastSyncedAt: 1, lastInsights: 1 }).toArray();

  const totalBudget = accounts.reduce((s, a) => s + (Number(a.spendCap || 0) / 100), 0);

...
export default function ReportsPage() {
  ...
  const loadOverview = useCallback(async () => {
    try { const res = await fetch("/api/admin/reports?type=overview"); const data = await res.json(); if (data.success) setOverview(data.data); } catch {}
  }, []);
  ...
  const loadFinancial = useCallback(async (period, start, end) => {
    try {
      ...
      const res = await fetch(url); const data = await res.json(); if (data.success) setFinancial(data.data);
    } catch {}
  }, []);
  ...
  const handleExport = async (type, format) => {
    ...
    try {
      ...
      if (format === "csv") {
        ...
        link.href = URL.createObjectURL(blob); link.download = `${type}_report_${range.start}_to_${range.end}.csv`; link.click(); URL.revokeObjectURL(link.href);
        await Swal.fire({ icon: "success", title: "CSV downloaded", timer: 1000, showConfirmButton: false });
      } else if (format === "pdf") {
        const html = await res.text(); const win = window.open("", "_blank"); win.document.write(html); win.document.close(); win.focus(); win.print();
      }
    } catch (err) { await Swal.fire({ icon: "error", title: "Export failed", text: err.message });
    } finally { setExporting(false); }
  };

  const formatMoney = (val) => { const n = Number(val || 0); if (!Number.isFinite(n)) return "0.00"; return n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }); };
  ...
  const StatCard = ({ label, value, sub, icon: Icon, color }) => (
    <div className="bg-white rounded-xl border border-slate-200/90 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`p-2 ${color || "bg-slate-50 text-slate-600"} rounded-lg`}><Icon size={18} /></span>
      </div>
  ...
  if (loading && !overview) return <div className="text-slate-500 py-10 text-center">Loading reports...</div>;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm">
            ...
            <input type="date" value={dateRange.start || range.start} onChange={(e) => setDateRange((p) => ({ ...p, start: e.target.value }))} className="border-0 bg-transparent text-sm w-[130px]" />
            <span className="text-slate-300">&mdash;</span>
            <input type="date" value={dateRange.end || range.end} onChange={(e) => setDateRange((p) => ({ ...p, end: e.target.value }))} className="border-0 bg-transparent text-sm w-[130px]" />
          </div>
          <button onClick={handleRefresh} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-2 text-sm font-medium hover:bg-slate-50 transition"><RefreshCw size={15} className={loading ? "animate-spin" : ""} /></button>
        </div>
      </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
              ...
              {["daily", "weekly", "monthly"].map((p) => (
                <button key={p} onClick={() => setFinancialPeriod(p)} className={`px-3 py-1.5 text-xs font-medium rounded-lg transition ${financialPeriod === p ? "bg-[#E05305] text-white" : "bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>{p.charAt(0).toUpperCase() + p.slice(1)}</button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleExport("financial", "csv")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><Download size={13} /> CSV</button>
              <button onClick={() => handleExport("financial", "pdf")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><FileText size={13} /> PDF</button>
            </div>
          </div>
          {financial.summary && (
      ...
      {activeTab === "users" && (
        <div>
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => handleExport("users", "csv")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><Download size={13} /> CSV</button>
              <button onClick={() => handleExport("users", "pdf")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><FileText size={13} /> PDF</button>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex flex-wrap items-center gap-4">
              {adSpend.summary && (<><span className="text-sm text-slate-500">{adSpend.summary.totalAccounts} accounts</span><span className="text-sm text-slate-500">Budget: <strong>${formatMoney(adSpend.summary.totalBudget)}</strong></span><span className="text-sm text-slate-500">Spent: <strong>${formatMoney(adSpend.summary.totalSpent)}</strong></span><span className="text-sm text-slate-500">Utilization: <strong className={adSpend.summary.utilization > 80 ? "text-red-600" : "text-emerald-600"}>{adSpend.summary.utilization.toFixed(1)}%</strong></span></>)}
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => handleExport("ad-spend", "csv")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><Download size={13} /> CSV</button>
              <button onClick={() => handleExport("ad-spend", "pdf")} disabled={exporting} className="border border-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-slate-50 transition flex items-center gap-1.5 disabled:opacity-50"><FileText size={13} /> PDF</button>
            </div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {adSpend.rows.length > 0 ? adSpend.rows.map((a, i) => {
                    ...
                    return (<tr key={a._id || i} className="hover:bg-slate-50/40">
                      ...
                      <td className="py-3 px-4 text-xs text-slate-400">{a.lastSyncedAt ? new Date(a.lastSyncedAt).toLocaleDateString() : "-"}</td>
                    </tr>);

...
export default function MetaApiSettingsPage() {
  ...
  const handleSyncSpend = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        await Swal.fire({ icon: data.errors > 0 ? "warning" : "success", title: `Sync done: ${data.synced} synced, ${data.errors} errors`, timer: 2000, showConfirmButton: false });
        loadSyncLogs();
      } else {
  ...
  if (loading) return <p className="text-slate-500">Loading Meta API Settings...</p>;
        ...
        {tabs.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeTab === t.id ? "bg-[#E05305] text-white shadow" : "text-slate-600 hover:bg-slate-50"}`}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <form onSubmit={handleSave} className="space-y-5 max-w-2xl">
            <div>
              ...
              <input name="bmId" defaultValue={settings?.businessManagerId || ""} placeholder="123456789012345" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm bg-white" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">App ID</label>
              <input name="appId" defaultValue={settings?.appId || ""} placeholder="123456789012345" className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm bg-white" />
            </div>
            <div>
            ...
            {connectionTest && (
              <div className={`rounded-lg p-4 text-sm ${connectionTest.success ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-red-50 text-red-800 border border-red-200"}`}>
      ...
      {activeTab === "accounts" && (
        <div>
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <button onClick={handleFetchAccounts} disabled={syncing} className="bg-[#E05305] text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-[#c84a04] transition disabled:opacity-50 flex items-center gap-2">
              <RefreshCw size={16} className={syncing ? "animate-spin" : ""} /> {syncing ? "Fetching..." : "Fetch from Meta BM"}
            </button>
            <button onClick={handleSyncSpend} disabled={syncing} className="border border-slate-200 text-slate-700 rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-slate-50 transition disabled:opacity-50 flex items-center gap-2">
              <RefreshCw size={16} className={syncing ? "animate-spin" : ""} /> {syncing ? "Syncing..." : "Sync Spend Data"}
            </button>

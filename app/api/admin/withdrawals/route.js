...
import { canUserWithdraw, debitUserBalance, getUserByUid } from "@/lib/userModel";
  ...
  try {
    ...
    if (status === "approved") {
      ...
      if (!canUserWithdraw(user)) {
        return NextResponse.json(
          { success: false, message: "Frozen accounts cannot withdraw." },
          { status: 403 }
        );
      }

      if (Number(user.availableBalance || 0) < withdrawal.amount) {
        return NextResponse.json(

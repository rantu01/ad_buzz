...
import { canUserWithdraw, getUserByUid } from "@/lib/userModel";
...
export async function POST(request) {
  try {
    ...
    if (!canUserWithdraw(user)) {
      return NextResponse.json({ success: false, message: "Account cannot withdraw." }, { status: 403 });
    }
    ...
    if (Number(user.availableBalance || 0) < numAmount) {
